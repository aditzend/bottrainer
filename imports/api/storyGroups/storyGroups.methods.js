...
export const createFailingTestsGroup = (projectId) => {
    ...
    Meteor.call('storyGroups.insert', {
        name: 'Failing tests',
        projectId,
...
export const createDefaultStoryGroup = async (projectId) => {
    if (!Meteor.isServer) throw Meteor.Error(401, 'Not Authorized');
    try {
        const storyGroupId = await Meteor.callWithPromise('storyGroups.insert', {
            name: 'Example group',
            projectId,
        });
        ...
        await Meteor.callWithPromise('stories.insert', {
            type: 'rule',
            steps: [{ intent: 'chitchat.bye' }, { action: 'utter_bye' }],
            title: 'Farewells',
            storyGroupId,
            projectId,
            events: ['utter_bye'],
...
Meteor.methods({
    async 'storyGroups.delete'(storyGroup) {
        ...
        await deleteResponsesRemovedFromStories(
            eventstoRemove,
    ...
    async 'storyGroups.rebuildOrder'(projectId) {
        ...
        const { storyGroups: order = [] } = Projects.findOne(
            { _id: projectId },
            { fields: { storyGroups: 1 } }
        );
        const storyGroups = StoryGroups.find(
            { projectId },
            { fields: { _id: 1, pinned: 1, name: 1 } }
        ).fetch();

        const newOrder = storyGroups
            .sort(
                (a, b) =>
                    order.findIndex((id) => id === a._id) -
                    order.findIndex((id) => id === b._id)
            )
