        ...
        return (
            <DocumentTitle title={projectName}>
                <Menu vertical inverted pointing className='project-menu' data-cy='project-menu'>
                    <Menu.Item>
                        <Menu.Header style={{ marginBottom: '20px' }}>Proyecto</Menu.Header>
                    ...
                    <Can I='incoming:r' projectId={projectId}>
                        <Link to={`/project/${projectId}/incoming`}>
                            <Menu.Item name='Entrantes' icon='inbox' data-cy='incoming-sidebar-link' />
                        </Link>
                    </Can>
                    <Can I='responses:r' projectId={projectId}>
                        <Link to={`/project/${projectId}/responses`}>
                            <Menu.Item name='Respuestas' icon='comment' />
                        </Link>
                    </Can>
                    ...
                    {canViewProjectsTab && (
                        <Link to={`/project/${projectId}/settings`}>
                            <Menu.Item name='Config' icon='setting' data-cy='settings-sidebar-link' />
                        </Link>
                    )}
                    <a href={settingsReady ? settings.settings.public.docUrl : ''} target='_blank' rel='noopener noreferrer'>
                        <Menu.Item name='Docs' icon='question' />
                    </a>
                    <Divider inverted />
                    ...
                    || isUserPermissionGlobal(Meteor.userId(), 'projects:r')) && (
                        <Link to='/admin/'>
                            <Menu.Item name='Panel Admin' icon='key' />
                        </Link>
                    )}
                    <Link to='/login'>
                        <Menu.Item data-cy='signout' name='Salir' icon='sign-out' />
                    </Link>
